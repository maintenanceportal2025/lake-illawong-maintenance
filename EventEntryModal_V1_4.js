 ...
 *     added to REFERENCE_TYPES, Reference field optional, generic label.
 *     Reduces friction -- Board can record evidence exists without being
 *     forced to specify location immediately; ExtendedNotes or a later
 *     edit can capture that detail if relevant.
 *
 *   V1.3 (Jun 2026): Visibility changed from single-select to multi-select.
 *     Checkboxes replace the dropdown -- Board can tick any combination.
 *     Dispute: Party A, Party B, Board checkboxes.
 *     Complaint: Resident, Board checkboxes.
 *     "Shared" and "All" removed as distinct options -- achieved by
 *     ticking multiple boxes (e.g. Party A + Party B = old "Shared").
 *     Stored and transmitted as comma-separated string, e.g. "Party A,Board".
 *     Validation requires at least one box checked.
 *     _save reads checked boxes via querySelectorAll, joins with comma.
 *     _reset clears all checkboxes and rebuilds the set per matterType.
 *
...
const EventEntryModal = (function () {
  ...
  const REFERENCE_TYPES = ['Meeting', 'Phone Call', 'Inspection', 'Evidence Received'];
  ...
  function _reset() {
    ...
    // Visibility checkboxes -- rebuilt per matterType each time modal opens
    ...
    if (visGroup) {
      ...
      opts.forEach(function (o, idx) {
        ...
        wrap.className = 'eem-checkbox-label';
        wrap.innerHTML = '<input type="checkbox" id="' + id + '" value="' + o + '" class="eem-checkbox" /> ' + o;
        visGroup.appendChild(wrap);
      });
    }

    // Clear fields
    ['eem-event-type', 'eem-summary',
  ...
  function _onTypeChange() {
    ...
    _showPanel('eem-panel-reference', isReference);
    _showPanel('eem-panel-notes',     !isEmail); // ExtendedNotes always shown except email (already has it)

    // Reference label always optional now -- no mandatory special case
    var refLabel = document.getElementById('eem-reference-label');
    if (refLabel) {
      refLabel.innerHTML = 'Reference <span class="eem-optional">(optional)</span>';
    }
  ...
  function _save() {
    ...
    var visBoxes    = document.querySelectorAll('#eem-visibility-group input[type="checkbox"]:checked');
    var visibility  = Array.prototype.map.call(visBoxes, function (cb) { return cb.value; }).join(',');
    ...
    else if (!visibility) err = 'Please select at least one visibility option.';
  ...
  function _inject() {
    ...
    style.textContent = [
      ...
      '.eem-checkbox-group { display:flex; gap:16px; flex-wrap:wrap; }',
      '.eem-checkbox-label { display:flex; align-items:center; gap:6px; font-size:0.85rem; color:white; cursor:pointer; }',
      '.eem-checkbox { width:16px; height:16px; cursor:pointer; accent-color:#0d9488; }',
    ...
    div.innerHTML = [
      ...
      '  <div class="eem-field">',
      '    <label class="eem-label">Visibility <span class="eem-required">*</span></label>',
      '    <p style="font-size:0.75rem;color:rgba(255,255,255,0.45);margin:0 0 8px;">Select all that apply.</p>',
      '    <div id="eem-visibility-group" class="eem-checkbox-group"></div>',
      '  </div>',
